...
#============================================================================================================================
    ...
    # This command automatically inserts "25.04" support into the nghdl script
...
#============================================================================================================================
    ...
    # Remove the missing libcanberra package from the inner script 
    # This command finds "libcanberra-gtk-module" and deletes it from the file

    sed -i 's/libcanberra-gtk-module //g' install-nghdl-scripts/install-nghdl-24.04.sh
...
#============================================================================================================================
    ...
    # --- FIX #5: Patch GHDL to accept LLVM 20 ---

    # Inject a command into the inner script that runs *after* tar extraction.
    # This command finds the "./configure" line in the inner script and inserts a sed command before it.
    # That inserted sed command will fix the GHDL 'configure' file right before it runs.

    sed -i '/\.\/configure --with-llvm/i \    sed -i "s/exit 1/echo ignored/g" configure' install-nghdl-scripts/install-nghdl-24.04.sh

    # --- FIX 4: Patch NGHDL Only (Specific Match) ---
   
    # This matches "../configure --enable-xspice" in the inner script.
    # It injects a command to patch the NGHDL 'configure' file to ignore GCC 14 errors.
    
    sed -i '/\.\.\/configure --enable-xspice/i \    sed -i "s/exit 1/echo ignored/g" ../configure' install-nghdl-scripts/install-nghdl-24.04.sh
...
#============================================================================================================================
    ...
    # ----  ISSUE_2 FIX #2 STARTS HERE ---

        # sudo add-apt-repository -y "ppa:$kicadppa"                # <----- COMMENTED OUT
        # sudo apt-get update                                       # <----- COMMENTED OUT

    # --- ISSUE_2 FIX #2 ENDS HERE ---
